...
String FileList() {
  ...
  String output = "--filelist-- {\"filelist\":[";
  if(root.isDirectory()){
      File file = root.openNextFile();
      while(file){
          if (output != "--filelist-- {\"filelist\":[") {
            output += ',';
          }
          output += "{\"type\":\"";
          output += (file.isDirectory()) ? "dir" : "file";
...
void readFilek(String FFF){
  File file = FILESYSTEM.open(FFF);
  if (!file) {Serial.println(F("Failed to read file"));return;}
